...
const fragmentShader = `
  ...
  void main() {
    float intensity = smoothstep(-0.1, 0.3, vElevation);
    vec3 color = mix(vec3(0.1, 0.3, 0.5), vec3(0.8, 0.9, 1.0), intensity);
    gl_FragColor = vec4(color, 1.0);
  }
`;

const PerlinDots = React.memo(() => {
  const { camera } = useThree();
  const axisRef = useRef(new THREE.Vector3(0, 1, 0)); // Y-axis
  ...
  useFrame((state, delta) => {
    ...
    // Apply the rotation
    camera.setRotationFromAxisAngle(axisRef.current, angleRef.current);
  ...
  return (
    <points ref={points} rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.5, 0]}>
...
export const Hero: React.FC = React.memo(() => {
  return (
    <div className='h-full w-full fixed top-0 left-0 -z-10'>
      <Canvas camera={{ position: [0, 1, 1], fov: 30 }}>
